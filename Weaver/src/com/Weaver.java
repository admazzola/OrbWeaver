...
public class Weaver extends Thread{
	...
	//this is a possible result of the library! This is the whole bread and butter
	...
	public void receiveFileChunkMessage(NodeFileChunkMessage nodeFileChunkMessage) {
			if(!isSeeding){
				this.getRegisteredOrb().getChunkManager().receiveChunk(nodeFileChunkMessage.getChunk() );
	...
	public void receiveFileChunkRequestMessage(NodeFileChunkRequestMessage nodeFileChunkRequestMessage) {
		if(isSeeding){
			
				chunkRequestQueueLock.lock();
			try{
				queuedChunkRequest.add( new QueuedChunkRequest(nodeFileChunkRequestMessage.chunkId, nodeFileChunkRequestMessage.senderInfo  )  );
			}finally{
				chunkRequestQueueLock.unlock();
			}
			
			System.out.println("Got file chunk request by "+  nodeFileChunkRequestMessage.senderInfo);
